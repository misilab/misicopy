...
struct SessionSnapshot: Codable, Sendable, Hashable {
    ...
    var etaSeconds: Int?
    /// Work-budget fields mirror CopyStats.workBudget / workDone so the
    /// iPhone uses the same progress formula as the Mac UI (which accounts
    /// for verification passes). Defaults to 0 so older Mac builds that
    /// don't populate these fields are still decoded without error.
    ...
    /// the same order as `destinationNames`. Empty array on older Macs (the
    /// Codable default makes the field invisible to pre-1.8.1 clients).
    /// The iPhone uses this to display "X GB free of Y GB" on the locked
    /// screen and inside the app so the user knows whether the next dump
    /// will fit before plugging in another card.
    ...
    /// Prefers workBudget/workDone when available (matches the Mac UI bar
    /// which includes verification passes), falls back to bytesProcessed/
    /// bytesTotal for older Mac builds that don't populate the work fields.
    var progress: Double {
        if workBudget > 0 {
...
enum RemoteCommand: String, Codable, Sendable {
    ...
    case cancel
    case ping
    /// 1.8.1+ — relaunch the copy pipeline on the previously-failed files
    /// only. The Mac decides whether to honour it (no-op when the engine
    /// is mid-run or has no failures on file). Older Macs reject the
    /// rawValue with a Codable error; new iOS apps detect that and grey
    /// the retry button out.
    case retryFailed
}
